...
import { Udemy, Guvi, Cisco } from "../asset/svg/svg";
...
const CreateCertificate = (props) => {
    ...
    let certImage;
    switch (props.item.platform.toLowerCase()) {
        case 'udemy':
            certImage = <Udemy />;
            break;
        case 'guvi':
            certImage = <div>{props.item.platform}</div>; 
            break;
        case 'cisco':
            certImage = <div>{props.item.platform}</div>;
            break;
        default:
                certImage = <div>{props.item.platform}</div>; // Fallback text
                break;
    }
