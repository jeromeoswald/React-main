...
import UniversityImg1 from "../../Data/universityLogo1.png";
...
function Degree(props) {
    ...
    return (
        <div className={classes.degreeMain}>
            ...
            <div className={classes.degreeCard}>
                <div className={`${classes.degreeImage} centered`} style={{borderColor:uiColor}}>
                    <img src={UniversityImg1} alt="degree" srcset="" />
                </div>
                <Card className={classes.degreeWrapper}>
                    <div className={classes.degreeInfo}>
                        <h3 style={{ color: nonThemeColor }}>{EducationData.couseStartYear1} - {EducationData.courseEndYear1}</h3>
                        <h1 style={{ color: uiColor }}>{EducationData.collegeName1}</h1>
                        <h2 style={{ color: nonThemeColor }}>{EducationData.courseName1}</h2>
                    </div>
                    <ul className={classes.details}>
                        {EducationData.details1.map((item, index) =>
                            <li key={index}>{item}</li>
                        )}
                    </ul>
                </Card>
